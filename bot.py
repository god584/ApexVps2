...
# Main admin user ID
MAIN_ADMIN_ID = 1377766105367445636
# VPS User Role ID
VPS_USER_ROLE_ID = 1543554256743956561
# LXC distro/release template to use for VPS containers
LXC_DISTRO = "ubuntu"
LXC_RELEASE = "jammy"  # Ubuntu 22.04
LXC_ARCH = "amd64"
# SSH port range for containers
SSH_PORT_START = 10000

# Bot branding
BOT_AUTHOR = "BlackBelt"
DISCORD_INVITE = "https://discord.gg/HkQjJz7ptP"
...
class SafeEmbed(discord.Embed):
    """Embed subclass that auto-truncates field values and caps field count
    so Discord's per-field (1024) and total embed (6000) limits can never be
    exceeded by long error/output text."""
...
def create_embed(title, description="", color=0x1a1a1a, fields=None):
    # Guard against Discord's hard limits (description 4096, field value 1024,
    # total embed size 6000) so long error/output text can never break sending.
    if description and len(description) > 3500:
        description = description[:3500] + "\n...(truncated)"
    embed = SafeEmbed(title=f"▌ {title}", description=description, color=color)
    ...
    embed.set_footer(
        text=f"StrengthCloud | VPS Manager • Made by {BOT_AUTHOR} • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        icon_url=""
    )
    return embed
...
async def execute_lxc(command, timeout=120):
    """Execute an LXC command with timeout and error handling."""
    try:
...
async def lxc_exec(container_name, command, timeout=60):
    """Execute a command inside a running LXC container via lxc-attach."""
...
async def create_lxc_container(container_name, ram_mb, cpu_count, ssh_port, password, disk_gb=30):
    """
    Create and configure an LXC container as a VPS.
    - Uses tmate for SSH access (no IPv4 required).
    - Installs OpenSSH server, tmate, and sets root password.
    """
    # Clean up any stale/partial container left over from a previous failed attempt.
    # These are expected to "fail" when no container exists yet — that's normal,
    # so we run them quietly instead of through execute_lxc (which logs at ERROR level).
    async def _quiet_lxc(cmd):
        try:
            proc = await asyncio.create_subprocess_exec(
                *shlex.split(cmd),
    ...
    await _quiet_lxc(f"lxc-stop -n {container_name}")
    await _quiet_lxc(f"lxc-destroy -f -n {container_name}")
    # Fallback: if lxc-destroy didn't fully clean up (e.g. partial/corrupt container
    # left by a previous failed create), forcibly remove its directory on disk.
    stale_path = f"/var/lib/lxc/{container_name}"
    if os.path.exists(stale_path):
        try:
            shutil.rmtree(stale_path, ignore_errors=True)
            logger.info(f"Force-removed stale container directory: {stale_path}")
        except Exception as e:
            logger.warning(f"Could not force-remove {stale_path}: {e}")

    # Create the container from the Ubuntu download template
    create_cmd = (
        f"lxc-create -n {container_name} -t download -- "
        f"-d {LXC_DISTRO} -r {LXC_RELEASE} -a {LXC_ARCH}"
    )
    await execute_lxc(create_cmd, timeout=300)

    # Apply resource limits (memory + CPU) to the container config
    try:
        cpu_quota = int(float(cpu_count) * 100000)
        ...
        logger.warning(f"Could not apply resource limits to {container_name}: {e}")

    # Start the container
    await execute_lxc(f"lxc-start -n {container_name} -d", timeout=60)
    await asyncio.sleep(3)

    # Wait for the container to actually have working internet before trying
    # apt-get — fail fast with a clear message instead of a wall of apt errors
    # if the host's LXC bridge/NAT networking isn't set up.
    network_ok = False
    for _ in range(10):
        check_stdout, check_stderr, check_rc = await lxc_exec(
            container_name,
            "timeout 3 bash -c 'cat < /dev/null > /dev/tcp/archive.ubuntu.com/80' && echo OK || echo FAIL",
            timeout=10
        )
        if check_rc == 0 and "OK" in check_stdout:
            network_ok = True
            break
        await asyncio.sleep(2)
    if not network_ok:
        raise Exception(
            "Container has no internet access (host LXC networking isn't set up). "
            "On the host: ensure the lxc-net bridge is running (`systemctl enable --now lxc-net`), "
            "IPv4 forwarding is on (`sysctl -w net.ipv4.ip_forward=1`), and NAT/MASQUERADE is set "
            "for the lxcbr0 subnet."
        )

    # Install openssh-server, tmate, and configure SSH + root password.
    # Force IPv4 for apt since many hosts don't route IPv6 out of the bridge.
    #
    # IMPORTANT: sshd_config uses "first match wins" per keyword — many base
    # images (esp. cloud/minimal Ubuntu) already ship an uncommented
    # "PasswordAuthentication no" line. Just appending "yes" with >> gets
    # silently ignored because the earlier "no" line still wins. Fix:
    #   1) sed-replace any existing (commented or not) directive in place
    #   2) ALSO drop an override file into /etc/ssh/sshd_config.d/, which
    #      Ubuntu's default sshd_config Include's at the very top of the
    #      file — so it wins the first-match race regardless of what the
    ...
    setup_script = (
        ...
        "printf 'PermitRootLogin yes\\nPasswordAuthentication yes\\n' > /etc/ssh/sshd_config.d/99-vps-access.conf && "
...
async def get_tmate_session(container_name):
    ...
    # Kill any old tmate process and remove a stale socket file first —
    # a leftover socket from a previous session can make tmate misbehave.
    await lxc_exec(container_name, f"pkill tmate 2>/dev/null || true; rm -f {socket_path}", timeout=10)
    await asyncio.sleep(1)

    # Run each tmate step separately so a failure clearly shows which
    # step broke, instead of one masked error from a chained command.
    stdout, stderr, rc = await lxc_exec(
        container_name, f"tmate -S {socket_path} new-session -d", timeout=15
    )
    if rc != 0:
        raise Exception(f"tmate new-session failed: {stderr or stdout}")

    await asyncio.sleep(3)

    stdout, stderr, rc = await lxc_exec(
        container_name, f"tmate -S {socket_path} wait tmate-ready", timeout=20
    )
    if rc != 0:
        raise Exception(f"tmate wait tmate-ready failed: {stderr or stdout}")

    stdout, stderr, rc = await lxc_exec(
        container_name, f"tmate -S {socket_path} display -p '#{{tmate_ssh}}'", timeout=10
    )
    if rc != 0 or not stdout.strip():
        raise Exception(f"tmate session start failed: {stderr or 'no SSH string returned'}")
    return stdout.strip()
    ...
    while cpu_monitor_active:
        try:
            ...
            if cpu_usage > CPU_THRESHOLD:
                ...
                try:
                    for cname in [vps['container_name']
                                  for vps_list in vps_data.values()
                                  for vps in vps_list
                                  if vps.get('status') == 'running']:
                        subprocess.run(['lxc-stop', '-n', cname, '-t', '5'], check=False)
                    for vps_list in vps_data.values():
                        for vps in vps_list:
                            if vps.get('status') == 'running':
                                vps['status'] = 'stopped'
...
class ManageView(discord.ui.View):
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if action == 'reinstall':
            ...
            class ConfirmView(discord.ui.View):
                ...
                async def confirm(self, interaction: discord.Interaction, item: discord.ui.Button):
                    ...
                    try:
                        ...
                        try:
                            await execute_lxc(f"lxc-stop -n {self.container_name}")
                        except:
                            pass
                        await execute_lxc(f"lxc-destroy -f -n {self.container_name}")
                        ...
                        await create_lxc_container(self.container_name, ram_mb, original_cpu, 0, new_password, disk_gb=original_disk)
            ...
            await interaction.response.defer(ephemeral=True)
            try:
                await execute_lxc(f"lxc-start -n {container_name} -d")
                # Restart SSH inside container
                await lxc_exec(container_name, "pkill -x sshd 2>/dev/null || true; sleep 1; /usr/sbin/sshd || true", timeout=10)
                vps["status"] = "running"
                save_data()
                await interaction.followup.send(
...
async def create_vps(ctx, user: discord.Member, ram: int, cpu: int, disk: int = 30):
    ...
    await ctx.send(embed=create_info_embed("Creating VPS", f"Deploying LXC VPS for {user.mention}...\n🧠 RAM: `{ram}GB` | ⚙️ CPU: `{cpu} Core(s)` | 💾 Disk: `{disk}GB`"))

    try:
        await create_lxc_container(container_name, ram_mb, cpu, 0, password, disk_gb=disk)
...
async def delete_vps(ctx, user: discord.Member, vps_number: int, *, reason: str = "No reason"):
    ...
    try:
        try:
            ...
            pass
        await execute_lxc(f"lxc-destroy -f -n {container_name}")
...
async def buy_with_credits(ctx, plan: str, processor: str = "Intel"):
    ...
    await ctx.send(embed=create_info_embed("Processing Purchase", f"Deploying {plan} LXC VPS..."))

    try:
        await create_lxc_container(container_name, ram_mb, cpu_str, 0, password)
...
async def backup_vps(ctx, container_name: str):
    """Create an LXC snapshot clone of a VPS (Admin only)"""
    snapshot_name = f"{container_name}-backup-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    await ctx.send(embed=create_info_embed("Creating Backup", f"Cloning snapshot of `{container_name}`..."))
    try:
        await execute_lxc(f"lxc-copy -n {container_name} -N {snapshot_name} -s")
        await ctx.send(embed=create_success_embed("Backup Created", f"Snapshot `{snapshot_name}` created!"))
    except Exception as e:
...
async def restore_vps(ctx, container_name: str, snapshot_name: str):
    """Restore a VPS from an LXC snapshot clone (Admin only)"""
    ...
    try:
        ...
        # Recreate from snapshot clone
        await execute_lxc(f"lxc-copy -n {snapshot_name} -N {container_name}")
        await execute_lxc(f"lxc-start -n {container_name} -d")
        await asyncio.sleep(2)
        await lxc_exec(container_name, "pkill -x sshd 2>/dev/null || true; sleep 1; /usr/sbin/sshd || true", timeout=10)
...
async def list_snapshots(ctx, container_name: str):
    """List LXC snapshot clones for a VPS (Admin only)"""
    try:
        proc = await asyncio.create_subprocess_exec(
            "lxc-ls", "-1",
            stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        ...
        snapshots = [img for img in all_containers if img.startswith(container_name + "-backup-")]
        if snapshots:
...
async def execute_command(ctx, container_name: str, *, command: str):
    ...
    try:
        stdout, stderr, rc = await lxc_exec(container_name, command, timeout=30)
...
def build_help_pages(is_user_admin, is_user_main_admin):
    ...
    extras_embed.add_field(name="👑 Made by", value=(
        f"This bot was made by **{BOT_AUTHOR}**\n"
        f"Join our Discord: {DISCORD_INVITE}"
    ), inline=False)
    pages["extras"] = extras_embed

    # Page: About Us
    about_embed = create_embed("ℹ️ About Us", "Everything about this bot & where to find us:", 0x5865F2)
    about_embed.add_field(name="👑 Made by", value=f"**{BOT_AUTHOR}**", inline=True)
    about_embed.add_field(name="🔗 Discord Server", value=f"{DISCORD_INVITE}", inline=True)
    about_embed.add_field(
        name="📦 What we offer",
        value="StrengthCloud | VPS Manager lets you deploy, manage and share LXC-based VPS containers directly from Discord.",
        inline=False
    )
    pages["about"] = about_embed
    ...
    if is_user_admin:
        ...
        admin_embed.add_field(name="🖥️ VPS Control", value=(
            "`!create @user <ram_GB> <cpu_cores> <disk_GB>` — Create custom LXC VPS\n"
        ...
        admin_embed.add_field(name="💾 Backup & Restore", value=(
            "`!backup-vps <container>` — Create LXC snapshot clone\n"
...
async def show_help(ctx):
    ...
    options = [
        ...
        discord.SelectOption(label="ℹ️ About Us", description="Made by, Discord link", value="about", emoji="ℹ️"),
    ]
    if is_user_admin:
...
@bot.command(name='about', aliases=['aboutus'])
async def about_bot(ctx):
    """Show information about this bot"""
    embed = create_embed("ℹ️ About Us", "Everything about this bot & where to find us:", 0x5865F2)
    embed.add_field(name="👑 Made by", value=f"**{BOT_AUTHOR}**", inline=True)
    embed.add_field(name="🔗 Discord Server", value=f"{DISCORD_INVITE}", inline=True)
    embed.add_field(
        name="📦 What we offer",
        value="StrengthCloud | VPS Manager lets you deploy, manage and share LXC-based VPS containers directly from Discord.",
        inline=False
...
async def ping_vps(ctx, vps_number: int):
    ...
    try:
        proc = await asyncio.create_subprocess_exec(
            "lxc-info", "-n", container, "-s", "-H",
            stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
        stdout, _ = await asyncio.wait_for(proc.communicate(), timeout=10)
        elapsed = int((time.time() - start) * 1000)
        is_running = stdout.decode().strip().upper() == "RUNNING"
        if is_running:
...
async def uptime_vps(ctx, vps_number: int):
    ...
    nickname = vps.get("nickname", f"VPS #{vps_number}")
    try:
        # Get the container's init PID via lxc-info
        pid_proc = await asyncio.create_subprocess_exec(
            "lxc-info", "-n", container, "-p", "-H",
            stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
        pid_stdout, _ = await asyncio.wait_for(pid_proc.communicate(), timeout=10)
        pid_str = pid_stdout.decode().strip()
        if not pid_str or pid_str == "-1":
            raise Exception("Container is not running")

        # Use ps to get elapsed seconds since the init process started
        ...
        elapsed_seconds = int(ps_stdout.decode().strip())
        started_at = datetime.utcnow() - dt_timedelta(seconds=elapsed_seconds)
        ...
        embed.add_field(name="Started At", value=f"`{started_at.strftime('%Y-%m-%d %H:%M:%S')} UTC`", inline=False)
        await ctx.send(embed=embed)
    except Exception as e:
